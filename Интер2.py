...
class MainWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.btn_pan.clicked.connect(lambda: None)
...
class AdvancedInterferometerApp(QMainWindow):
    ...
    def load_data_dropped(self, file_path):
        # Используем ту же логику, что и load_data()
        try:
...
_orig_init = AdvancedInterferometerApp.__init__
def new_init(self, *args, **kwargs):
    _orig_init(self, *args, **kwargs)
    self.setAcceptDrops(True)
    self.drag_overlay = DragOverlay(self.central_widget)
    self.drag_overlay.raise_()
AdvancedInterferometerApp.__init__ = new_init

def resizeEvent(self, event):
    super(AdvancedInterferometerApp, self).resizeEvent(event)
    self.drag_overlay.setGeometry(self.central_widget.rect())
AdvancedInterferometerApp.resizeEvent = resizeEvent

def dragEnterEvent(self, event):
    if event.mimeData().hasUrls() and event.mimeData().urls()[0].toLocalFile().lower().endswith(('.txt','.csv','.xlsx')):
        event.acceptProposedAction()
        self.drag_overlay.showOverlay()
    else:
        event.ignore()
AdvancedInterferometerApp.dragEnterEvent = dragEnterEvent

def dragLeaveEvent(self, event):
    self.drag_overlay.hideOverlay()
    event.accept()
AdvancedInterferometerApp.dragLeaveEvent = dragLeaveEvent

def dropEvent(self, event):
    if event.mimeData().hasUrls():
        path = event.mimeData().urls()[0].toLocalFile()
        if path.lower().endswith(('.txt','.csv','.xlsx')):
            self.load_data_dropped(path)
            self.drag_overlay.hideOverlay()
            event.acceptProposedAction()
            return
    event.ignore()
AdvancedInterferometerApp.dropEvent = dropEvent
